...
export async function buildNewsBundle({ ticker, baselineDate, openKey, model, useLlm=true, articleLimit=5, finnhubKey, fmpKey }){
  ...
  try{
    ...
    let articles = [];
    try{
      const fmpArticles = await fetchFmpArticles({ ticker, baselineDate, key: fmpKey, limit: articleLimit * 4 });
      articles = uniqArticles([...articles, ...fmpArticles]);
    }catch(err){
      console.warn('[News] FMP news failed', err.message);
    }
    try{
      const gdeltArticles = await fetchGdeltArticles({ ticker, keywords, baselineDate });
      articles = uniqArticles([...articles, ...gdeltArticles]);
    }catch(err){
      console.warn('[News] GDELT failed', err.message);
    }
    if((!articles || articles.length < articleLimit) && finnhubKey){
      try{
        const fhArticles = await fetchFinnhubNews({ ticker, baselineDate, key: finnhubKey });
        articles = uniqArticles([...articles, ...fhArticles]);
      }catch(err){
