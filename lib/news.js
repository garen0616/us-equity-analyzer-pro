...
export async function buildNewsBundle({ ticker, baselineDate, openKey, model, useLlm=true, articleLimit=5, finnhubKey }){
  ...
  try{
    ...
    let articles = [];
    try{
      articles = await fetchGdeltArticles({ ticker, keywords, baselineDate });
    }catch(err){
      console.warn('[News] GDELT failed', err.message);
    }
    if((!articles || !articles.length) && finnhubKey){
      try{
        articles = await fetchFinnhubNews({ ticker, baselineDate, key: finnhubKey });
      }catch(err){
